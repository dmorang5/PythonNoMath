...
def alineamiento(imagen, ancho, alto):
    ...
    contorno, jerarquía = cv.findContours(umbral, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    ...
    if imagen_A6 is not None:
        ...
        for c_2 in contorno2:
            ...
            if area < 9300 and area > 8000:
                ...
                cv.putText(imagen_A6, "$/. 0.05",(x,y) , font, 0.75, (0,255,0),2)
                suma1 = suma1 + 0.2
            
            if area < 7800 and area > 6500:
                font=cv.FONT_HERSHEY_SIMPLEX
                cv.putText(imagen_A6, "$/. 0.10",(x,y) , font, 0.75, (0,255,0),2)
                suma2 = suma2 + 0.1
        total = suma1 + suma2
